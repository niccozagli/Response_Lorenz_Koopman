...
class EDMD_CHEB(BaseEDMD):
    ...
    def evaluate_koopman_eigenfunctions_reduced(self, data: np.ndarray, tsvd_regulariser: TSVD) -> np.ndarray:
        if tsvd_regulariser.Ur is None or tsvd_regulariser.reduced_right_eigvecs is None:
...
class Tikhonov():
    ...
    def tikhonov(self,edmd : EDMD_CHEB) -> np.ndarray:
        if edmd.G is not None and edmd.A is not None:
            ...
            raise ValueError("Matrices G and A are not set. Run `perform_edmd()` first.")
...
class TSVD():
    ...
    def get_spectral_properties(self):
        ...
        else: raise RuntimeError("You must call `decompose()` before computing spectral properties.")

    def map_eigenvectors(self,eigenvectors):
        if self.Ur is not None:
            return self.Ur @ eigenvectors
        else: RuntimeError("You must call `decompose()` before mapping eigenvectors.")
